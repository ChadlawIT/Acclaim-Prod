...
**Dedicated post-create endpoint:** SOS pushes restriction changes shortly AFTER creation as a separate one-shot call to `POST /api/external/cases/:externalRef/restrictions` — deliberately NOT part of the periodic sync, so re-syncs never touch restrictions. It accepts EITHER OR BOTH `hideFromUsers` (add restriction) and `showToUsers` (lift restriction / give access back); at least one is required (400 otherwise). Each accepts array / single value / comma-separated. Two shared helpers in routes.ts: `applyExternalCaseRestrictions()` (hide) and `liftExternalCaseRestrictions()` (show). Both de-dupe by resolved user id and are idempotent-aware via boolean-returning storage methods (`addCaseAccessRestriction` true=new row; `removeCaseAccessRestriction` true=row deleted), so audits only fire on real changes; no-ops bucket into `alreadyRestrictedUserIds`/`alreadyAllowedUserIds`. Hides run before shows, so if a user is in both lists the lift wins (ends with access) — this is the "correction" flow (e.g. wrong size: hide B + show A swaps who's blocked).

**Why SOS needs explicit show, not replace:** plain additive hide can't undo a mistaken restriction — re-pushing a corrected hide list leaves the previously-hidden user still blocked. SOS therefore sends an explicit `showToUsers` to give access back, rather than "replace mode" (replace would also clobber an admin's portal lift on re-push).
