...
`TL0016`, `AC0123`). **Confirmed from prod (visual timeline): the code is in the `activity_type`
column, and `description` holds the human label** (e.g. `activity_type="TL0001"`,
`description="Case created"`). It can also appear embedded in `description` text for some feeds, so
**check/match BOTH `description` and `activity_type`** — do not assume one field. The wording around
the code can change; the code itself is the stable identifier.
...
- For DB efficiency, pre-filter with `(description ILIKE '%TL0001%' OR activity_type ILIKE
  '%TL0001%')`, then apply the strict regex in JS to both fields — the ILIKE alone over-matches
  (e.g. `TL00010`).

**Why:** these activities come only from SOS; the code is the contract, the surrounding text is not.
A description-only match silently misses cases whose code lives in `activity_type`, causing the
recovery report to fall back to the portal date and falsely flag "No start date".
